...
public class Assembler {
	...
	private boolean readInputFile(String macroFile) {
		...
		String filePath = "files/srcCodes/" + macroFile;
		try {
	...
	private SrcLine parseLine(String line) {
		...
		if (!trimmed.isEmpty()) {
			 ...
			 * if (trimmed.charAt(0) != '*') {
			 * 
			 * // Tokenização básica por espaços String[] toks = trimmed.split("\\s+"); int
			 * idx = 0;
			 * 
			 * // label? (present se NÃO for opcode ou diretiva) if
			 * (!isOpcodeOrDir(toks[idx])) { sl.label = toks[idx++]; }
			 * 
			 * sl.opcode = toks[idx++].toUpperCase(); if (idx < toks.length) sl.op1 =
			 * toks[idx++]; if (idx < toks.length) sl.op2 = toks[idx];
			 * 
			 * 
			 * }
			 */
	...
	/*
	 * private int valueOf(String token) { token = token.trim(); if
	 * (token.startsWith("H'")) return Integer.parseInt(token.substring(2,
	 * token.length()-1), 16); if (token.startsWith("@")) return
	 * Integer.parseInt(token.substring(1)); // literal decimal simplificado return
	 * Integer.parseInt(token); // decimal }
	 */
